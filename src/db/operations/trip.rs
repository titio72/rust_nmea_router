...
#[cfg(test)]
mod tests {
    use crate::db::test_helpers::{add_test_trip, setup_db, add_test_vessel_status, fetch_vessel_status_by_id};
    use std::{ops::{Add, Sub}, time::{Duration, SystemTime}};
    ...
    fn test_update_trip_description() {
        ...
        // Insert a test trip
        let trip_id: u32 = add_test_trip(&db, "Original Description".to_string(), t, t.add(Duration::from_secs(ONE_HOUR_S)), 0.0, 0.0, 0, 0, ONE_HOUR_S * 1000).expect("Failed to insert test trip");  

        // Update the trip description
        db.update_trip_description(trip_id as i64, "Updated Description").expect("Failed to update trip description");

        // Fetch the trip and verify the description was updated
        let fetched_trip = db.fetch_trip(trip_id).expect("Failed to fetch trip").expect("Trip not found");
        assert_eq!(fetched_trip.description, "Updated Description", "Trip description should be updated");
    }
    ...
    fn test_delete_trip() {
        ...
        let trip_id: u32 = add_test_trip(&db, "Test Trip".to_string(), t, t.add(Duration::from_secs(trip_lenght_h * ONE_HOUR_S)), 0.0, 0.0, 0, 0, trip_lenght_h * ONE_HOUR_S * 1000).expect("Failed to insert test trip");  
        ...
        while report_time < t.add(Duration::from_secs(trip_lenght_h * ONE_HOUR_S)) {
            let status_id = add_test_vessel_status(&db, report_time, 43.0, 11.0, 0.0, 0.0, Some(5.0), Some(9.0), true, crate::utilities::EngineStatus::Off, 0.0, 0, None, Some(270.0)).expect("Failed to insert test vessel status");
        ...
        assert!(db.fetch_trip(trip_id).expect("Failed to fetch trip").expect("Trip not found").id == trip_id, "Inserted trip ID should match fetched trip ID");
        let status_id = add_test_vessel_status(&db, report_time, 43.0, 11.0, 0.0, 0.0, Some(5.0), Some(9.0), true, crate::utilities::EngineStatus::Off, 0.0, 0, None, Some(270.0)).expect("Failed to insert test vessel status");
        reports_ids.push(status_id);
        ...
        let x = db.fetch_trip(trip_id).expect("Failed to fetch trip after deletion");
        assert!(x.is_none(), "Trip record should not exist after deletion");
        reports_ids.iter().enumerate().for_each(|(i, report_id)| {
            let status = fetch_vessel_status_by_id(&db, *report_id).expect("Failed to fetch vessel status after deletion");
            if i == 0 || i == reports_ids.len() - 1 {
                // First and last records should still exist (outside trip range)
                assert!(status.is_some(), "Vessel status record outside trip range should not be deleted");
            } else {
                // Records within trip range should be deleted
                assert!(status.is_none(), "Vessel status record within trip range should be deleted");
            }
        });
    ...
    fn test_trim_trip() {
        ...
        // add test data for a trip with vessel status reports starting before the trip and ending after the trip to test that only records within the trip range are deleted
        // Insert a test trip with associated data
        let trip_id: u32 = add_test_trip(&db, "Test Trip".to_string(), 
            t_start.sub(Duration::from_secs(6 * ONE_HOUR_S)), 
            t_end.add(Duration::from_secs(6 * ONE_HOUR_S)), 
            0.0, 0.0, 0, 0, trip_lenght_h * ONE_HOUR_S * 1000).expect("Failed to insert test trip");  
        // Insert vessel status records every 30 minutes for 8 hours, alternating moored and not moored
        ...
        while report_time < t_end {
            let status_id = add_test_vessel_status(&db, report_time, 43.0, 11.0, 0.0, 0.0, Some(5.0), Some(9.0), false, crate::utilities::EngineStatus::On, 0.0, 0, None, Some(270.0)).expect("Failed to insert test vessel status");
            reports_ids.push((status_id, report_time));
        ...
        if let Some(x) = db.fetch_trip(trip_id).expect("Failed to fetch trip after trim") {
            ...
            let start_diff = actual_start.duration_since(expected_start).unwrap_or_else(|_| expected_start.duration_since(actual_start).unwrap_or(Duration::from_secs(0)));
            let end_diff = actual_end.duration_since(expected_end).unwrap_or_else(|_| expected_end.duration_since(actual_end).unwrap_or(Duration::from_secs(0)));
            assert!(start_diff.as_secs() < 60, "Start timestamp differs by {} seconds", start_diff.as_secs());
            assert!(end_diff.as_secs() < 60, "End timestamp differs by {} seconds", end_diff.as_secs());
        } else {
            assert!(false, "Trip record should exist after trim");
        }
        ...
        reports_ids.iter().for_each(|(status_id, ts)| {
            if let Some(status) = fetch_vessel_status_by_id(&db, *status_id).expect("Failed to fetch vessel status after deletion") {
                assert!(status.timestamp >= expected_start.sub(leeway) && status.timestamp <= expected_end.add(leeway), "Vessel status record with timestamp {:?} should be within the new trip range", status.timestamp);
            } else {
                assert!(ts < &expected_start.add(leeway) || ts > &expected_end.sub(leeway), "Vessel status record with timestamp {:?} should not be deleted", ts);
            }
        });
    }
}