...
mod test_infrastructure_examples {
        ...
        populate_multi_leg_trip(
            ...
            vec![
                TestLeg::new(
                    ...
                    start_trip_time,
                    p1,
                    start_trip_time + Duration::from_secs(7 * 3600),
                    6.0,
                    45.0,
                    135.0,
                ),
                TestLeg::new(
                    p1,
                    start_trip_time + Duration::from_secs(9 * 3600),
                    p0,
                    start_trip_time + Duration::from_secs(16 * 3600),
                    5.5,
                    50.0,
                    270.0,
                ),
    ...
    fn test_basic_trip_insertion_and_retrieval() {
        ...
        assert_approx_equal(
            retrieved_trip.total_distance_sailed,
            10.5,
            0.001,
            "Sailed distance",
        );
        assert_approx_equal(
            retrieved_trip.total_distance_motoring,
            2.3,
            0.001,
            "Motored distance",
        );
    ...
    fn test_vessel_status_insertion_and_retrieval() {
        ...
        assert_approx_equal(
            retrieved.latitude.unwrap(),
            position.latitude,
            0.0001,
            "Latitude",
        );
        assert_approx_equal(
            retrieved.longitude.unwrap(),
            position.longitude,
            0.0001,
            "Longitude",
        );
    ...
    fn test_position_calculation_from_bearing() {
        ...
        assert_approx_equal(
            ...
            expected_lat,
            0.001,
            "Latitude after northward travel",
        );
        assert_approx_equal(
            end.longitude,
            start.longitude,
    ...
    fn test_simulated_sailing_trip() {
        ...
        let trip_id = insert_simulated_sailing_trip(
            &db, start_pos, end_pos, start_time, 6.0, // 6 knots
        ...
        assert!(
            trip.total_distance_sailed > 0.0,
            "Should have sailed distance"
        );
        assert_eq!(
            trip.total_distance_motoring, 0.0,
            "Should have no motoring distance"
        );
    ...
    fn test_simulated_motoring_trip() {
        ...
        let trip_id = insert_simulated_motoring_trip(
            &db, start_pos, end_pos, start_time, 7.0, // 7 knots
        ...
        assert_eq!(
            trip.total_distance_sailed, 0.0,
            "Should have no sailing distance"
        );
        assert!(
            trip.total_distance_motoring > 0.0,
            "Should have motoring distance"
        );
    ...
    fn test_moored_status_insertion() {
        ...
        let status_id = insert_moored_status(
            &db, position, timestamp, 3600000, // 1 hour
        )
    ...
    fn test_populate_sample_trips() {
        ...
        let trip_ids = populate_sample_trips(&db).expect("Failed to populate sample trips");
    ...
    fn test_database_reset() {
        ...
        // Verify trip exists
        let trip_before = fetch_trip_by_timestamp(&db, start_time).expect("Failed to fetch trip");
        ...
        // Verify trip is gone
        let trip_after = fetch_trip_by_timestamp(&db, start_time).expect("Failed to fetch trip");
    ...
    fn test_populate_multi_leg_trip() {
        ...
        let leg1_start = Position {
            latitude: 41.0,
            longitude: 2.0,
        };
        let leg1_end = Position {
            latitude: 41.3,
            longitude: 2.4,
        };
        ...
        let leg2_start = Position {
            latitude: 41.3,
            longitude: 2.4,
        };
        let leg2_end = Position {
            latitude: 41.6,
            longitude: 2.8,
        };
        ...
        let initial_moored =
            fetch_vessel_status_by_timestamp(&db, leg1_start_time - Duration::from_secs(300))
    ...
    fn test_multi_leg_trip_with_gap() {
        ...
        let gap_status = fetch_vessel_status_by_timestamp(&db, gap_time).expect("Query failed");
        ...
        if let Some(status) = gap_status {
            ...
            assert!(
                status.average_wind_speed_kn.is_some() || status.average_wind_speed_kn.is_none()
            );
        } else {
    ...
    fn test_import_trip_from_file() {
        ...
        let trip_file_path = "./test_files/trip_7.json".to_string();

        // Read JSON content from file
        let json_data = std::fs::read_to_string(&trip_file_path)
            .expect("Failed to read trip file - ensure ./test_files/trip_7.json exists");
        ...
        let total_distance_motoring: f64 =
            trip_row.get(3).expect("Missing total_distance_motoring");
        ...
        let vessel_status_count: (i64,) = conn
            .exec_first("SELECT COUNT(*) FROM vessel_status", ())
            .expect("Failed to count vessel statuses")
            .expect("Query failed");

        assert!(
            vessel_status_count.0 > 0,
            "Vessel status records should be imported"
        );

        // Verify environmental metrics were inserted
        let env_metrics_count: (i64,) = conn
            .exec_first("SELECT COUNT(*) FROM environmental_data", ())
            .expect("Failed to count environmental metrics")
            .expect("Query failed");

        assert!(
            env_metrics_count.0 > 0,
            "Environmental metric records should be imported"
        );
    }
    ...
    fn test_uuid_import_deduplication() {
        ...
        assert_eq!(
            count_with_uuid.0, 1,
            "Should have exactly 1 trip with that UUID after first import"
        );
        ...
        assert_ne!(
            first_id, second_id,
            "Re-import should produce a new DB row ID"
        );
        ...
        assert_eq!(
            count_after.0, 1,
            "Should still have exactly 1 trip with that UUID after re-import"
        );
        ...
        assert_eq!(
            desc.as_deref(),
            Some("Re-imported"),
            "Surviving trip should have updated description"
        );

        // Old ID must be gone
        let old_id_row: Option<mysql::Row> = conn
            .exec_first(
        ...
        assert!(
            old_id_row.is_none(),
            "Old trip record must have been deleted"
        );
    }
}
