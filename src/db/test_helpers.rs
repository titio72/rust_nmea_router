...
pub fn calculate_position_from_bearing(
    ...
    let lat2 = (lat1.sin() * angular_distance.cos() 
        + lat1.cos() * angular_distance.sin() * bearing_rad.cos()).asin();
    
    let lon2 = lon1 + (bearing_rad.sin() * angular_distance.sin() * lat1.cos())
...
#[allow(dead_code)]
pub fn generate_realistic_wind(base_speed_kn: f64, base_angle_deg: f64, variation: f64) -> (f64, f64) {
    use rand::Rng;
    ...
    if let Some(mut row) = row {
        ...
        let average_speed_kn: f64 = {
            match row.take::<mysql::Value, _>("average_speed_kn") {
                ...
                Some(v) => return Err(format!("Unexpected type for average_speed_kn: {:?}", v).into()),
            }
        };
        ...
        let total_distance_nm: f64 = {
            match row.take::<mysql::Value, _>("total_distance_nm") {
                ...
                Some(v) => return Err(format!("Unexpected type for total_distance_nm: {:?}", v).into()),
            }
        };
...
pub fn populate_multi_leg_trip(
    ...
    for (leg_idx, leg) in legs.iter().enumerate() {
        ...
        let leg_distance = haversine_distance_nm(
            leg.start_position.latitude, leg.start_position.longitude,
            leg.end_position.latitude, leg.end_position.longitude
        );
        
        let leg_duration = leg.end_time.duration_since(leg.start_time)
        ...
        let track = generate_track(leg.start_position, leg.end_position, leg_speed_kn, 30, leg.start_time);
        ...
        for i in 0..track.len() {
            ...
            let segment_time_ms = timestamp.duration_since(last_report_time)
                .unwrap_or(Duration::from_secs(0))
                .as_millis() as u64;
            ...
            let segment_speed = if segment_time_ms > 0 {
                ...
                if hours > 0.0 { segment_distance / hours } else { 0.0 }
            } else {
                0.0
            };
        ...
        if !is_last_leg {
            ...
            while current_time < next_leg_start {
                let remaining = next_leg_start.duration_since(current_time)
                    .unwrap_or(Duration::from_secs(0));
                ...
                if interval_ms > 0 {
                    ...
                    current_time = current_time + Duration::from_millis(interval_ms);
                } else {
                    break;
                }
            }
...
mod tests {
    ...
    fn test_calculate_position_from_bearing() {
        // Start at 0°N, 0°E
        let p0 = Position { latitude: 0.0, longitude: 0.0 };
        ...
        assert_approx_equal(p1.latitude, 1.0 / 60.0, 0.0001, "Latitude after moving north");
        assert_approx_equal(p1.longitude, 0.0, 0.0001, "Longitude should remain same");
    }

    #[test]
    fn test_generate_track() {
        let p0 = Position { latitude: 40.0, longitude: -70.0 };
        let p1 = Position { latitude: 40.0, longitude: -69.0 };
        ...
        assert_approx_equal(track[0].0.longitude, p0.longitude, 0.0001, "First point lon");
