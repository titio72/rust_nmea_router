...
use crate::utilities::{calculate_true_wind, EngineStatus, TimedQueue};
...
impl VesselMonitor {
    pub fn new(status_report_period: Duration, status_report_moored_period: Duration) -> Self {
        ...
        VesselMonitor {
            ...
            vmg_for_mooring: MooringDetectionQueue::new(
                MOORING_DETECTION_WINDOW,
                MOORING_ACCURACY,
                MOORING_THRESHOLD_VMG_KNOTS,
            ),
    ...
    pub fn process_position(&mut self, position_msg: &PositionRapidUpdate, timestamp: Instant) {
        ...
        let median_position = self.positions.get_rolling_median_position(
            TIME_WINDOW_POSITION_NOISE_FILTER,
    ...
    fn process_wind(&mut self, wind_msg: &nmea2k::pgns::WindData, timestamp: Instant) {
        ...
        if let Some(speed) = speed_sample {
            if speed.timestamp + Duration::from_secs(5) < timestamp {
                ...
                let (true_wind_speed_kn, true_wind_angle_deg) =
                    calculate_true_wind(wind_speed_kn, wind_angle_deg, speed.value);
                ...
                self.wind_angles.add_sample(
                    crate::utilities::normalize0_360(true_wind_angle_deg),
                    timestamp,
                );
            }
        }
    ...
    pub fn process_engine(
        &mut self,
        engine_msg: &nmea2k::pgns::EngineRapidUpdate,
        timestamp: Instant,
    ) {
        ...
        if new_status == self.engine_on {
            ...
            if self.engine_pending_status == Some(new_status) {
                ...
                if self
                    .engine_pending_since
                    .is_some_and(|t| timestamp.duration_since(t) >= Duration::from_secs(5))
                {
    ...
    /// Expected maximum rate is 10 Hz
    pub fn process_heading(
        &mut self,
        heading_msg: &nmea2k::pgns::VesselHeading,
        timestamp: Instant,
    ) {
        if heading_msg.reference == HeadingReference::Magnetic {
            ...
            if let Some(pos) = self.positions.get_latest_position() {
                ...
                self.headings
                    .add_sample(heading_msg.heading.to_degrees(), timestamp);
            }
        }
    }
    ...
    fn should_generate_event(&self, now: Instant) -> bool {
        ...
        now.duration_since(self.last_event_time) >= period
            && self.positions.len() >= MIN_SAMPLES_FOR_VALIDATION
    }
    ...
    pub fn generate_status(&mut self, now: Instant) -> Option<VesselStatus> {
        ...
        let start_sampling_time = self
            .positions
            .get_earliest_position_timestamp()
            .unwrap_or(now);
        let (number_of_samples, median_position) = self.positions.get_rolling_median_position(
            self.status_report_period,
            MIN_SAMPLES_FOR_VALIDATION,
            now,
        );
        let max_speed_kn = self
            .speeds
            .get_max(self.status_report_period, now)
            .unwrap_or(0.0);
        let is_moored = self.is_moored(now);
        let wind_speed_kn = self.wind_speeds.get_average(self.status_report_period, now);
        let wind_angle_deg = self
            .wind_angles
            .get_average_as_angle_deg(self.status_report_period, now);
        let max_wind_speed_kn = self.wind_speeds.get_max(self.status_report_period, now);

        let average_heading = self
            .headings
            .get_average_as_angle_deg(self.status_report_period, now);
...
mod tests {
    ...
    fn make_wind_sample(
        ...
        println!(
            "Encoding wind: {} kn ({:.3} m/s), angle {} deg ({:.3} rad)",
            speed_kn, speed_mps, angle_deg, angle_rad
        );
    ...
    fn test_wind_rolling_window() {
        ...
        assert!(
            !monitor.wind_speeds.is_empty(),
            "Expected wind speed samples"
        );
    ...
    use super::*;
    use nmea2k::pgns::{CogSogRapidUpdate, PositionRapidUpdate};
    ...
    fn test_vessel_status_creation() {
        ...
        for (ws, wa) in &wind_samples {
            let (tw_speed, tw_angle) =
                crate::utilities::calculate_true_wind(*ws, *wa, boat_speed_kn);
        ...
        monitor.last_event_time =
            std::time::Instant::now() - monitor.status_report_period - Duration::from_secs(1);
        ...
        println!(
            "Expected avg speed: {:.3}, got: {:.3}",
            expected_speed, speed
        );
        println!(
            "Expected avg angle: {:.3}, got: {:.3}",
