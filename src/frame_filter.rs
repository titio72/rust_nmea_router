...
pub fn should_process_frame_by_id(config: &Config, id: Identifier) -> bool {
    ...
    config.can.source_filter.should_accept(id.pgn(), id.source())
}
...
mod tests {
    ...
    fn create_test_config(pgn_source_map: HashMap<u32, u8>) -> Config {
        ...
        config.can.source_filter.pgn_source_map = pgn_source_map;
        config
    }
    ...
    fn test_should_process_frame_by_id_with_specific_filter() {
        ...
        assert!(should_process_frame_by_id(
            &config,
            create_identifier(129025, 22)
        ));
        assert!(should_process_frame_by_id(
            &config,
            create_identifier(127488, 5)
        ));

        // Matching PGN but different source - should reject
        assert!(!should_process_frame_by_id(
            &config,
            create_identifier(129025, 10)
        ));
        assert!(!should_process_frame_by_id(
            &config,
            create_identifier(127488, 10)
        ));

        // Different PGN not in map - should accept (no filter for this PGN)
        assert!(should_process_frame_by_id(
            &config,
            create_identifier(128259, 0)
        ));
    }
    ...
    fn test_should_process_frame_by_id_various_pgn_values() {
        ...
        assert!(should_process_frame_by_id(
            &config,
            create_identifier(60928, 0)
        ));
        assert!(should_process_frame_by_id(
            &config,
            create_identifier(129025, 1)
        ));
        assert!(should_process_frame_by_id(
            &config,
            create_identifier(130306, 2)
        ));
        assert!(should_process_frame_by_id(
            &config,
            create_identifier(127245, 3)
        ));

        // Same PGNs but wrong sources should be rejected
        assert!(!should_process_frame_by_id(
            &config,
            create_identifier(60928, 1)
        ));
        assert!(!should_process_frame_by_id(
            &config,
            create_identifier(129025, 0)
        ...
        assert!(!should_process_frame_by_id(
            &config,
            create_identifier(130306, 10)
        ));
        assert!(!should_process_frame_by_id(
            &config,
            create_identifier(127245, 5)
        ));
    }
