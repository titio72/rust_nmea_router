...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let vessel_monitor = VesselMonitor::new(config.database.vessel_status.interval_underway(), config.database.vessel_status.interval_moored());

    // Create time monitor
    let time_monitor = TimeMonitor::new(
    ...
    let environmental_status_handler = environmental_status_handler::EnvironmentalStatusHandler::new(&config.database.environmental);
